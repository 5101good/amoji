...
## D4 第一轮边界修补（2026-09-20）

限定修复四个审查问题：新预览请求立即撤销旧图，并以请求身份约束 load、勾选和确认；切换条目后版本加载完成前不展示旧详情操作；导入超时、连接丢失、缺失/损坏成功正文统一为 `PACK_OUTCOME_UNKNOWN`，只有结构化服务错误保留其业务 code/current；条目冲突继续编辑时迁移旧草稿图片的原始字节，不继承当前库条目的其他图片。草稿列表改为内容高度排列，保存后即时更新名称。

新增五个聚焦 DOM/RPC 回归先失败后通过；另一个隔离真实共享服务测试完成“上传改图 → 保存预览 → 并发更新 → 确认冲突 → 继续编辑 → 确认新版本”，最终 `visual.primary.sha256` 与上传图片一致。受影响的既有确认、建议失效和包未知结果检查三项通过；typecheck、build:dsh 与 diff 检查通过。没有重跑 39 项套件、修改用户服务或调用模型。root 已独立取得正常流程的真实 UI 证据，见 [dsh UI 真机记录](dsh-ui-live-2026-09-20.md)；此处不把本轮 DOM 检查当作重新进行像素验收。
