...
- 代码子任务只允许本地可逆实现与测试，不重启用户服务、不运行有费用模型；根代理按 Task 3 和用户已有授权负责备份、空闲检查、本地激活及必要的低成本验证。所有任务不推远端。
...
2026-09-21：三个任务均完成。独立 QA 浏览器地址受到客户端拦截；最终使用用户日常 3080 页面，在实际 Edge 中完成验收。已安装到 web profile；仅使用 deepseek-v4-flash 新建验证会话发送一次办公“谢谢”。详细结果见 `docs/acceptance/2026-09-21-expression-appearance.md`。main 与远端保持不变。
