...
|---|---|---|
| T01 用户单发表情 | D1真实零文字输入/自然回复；1bf2dac暂停下手动A1/A2各completed | 真实Host通过；最终选择器普通A→B→A切换已真机，未知投递时序见自动故障验证 |
...
| T05 创建/建议/确认 | D4浏览器上传、建议修改采用、预览load与明确确认 | 主旅程通过；最终包清空可选字段/预览/确认已真机通过 |
| T06 建议不可用手工继续 | 自动故障与保留输入合同通过 | 真实核心断连导致建议不可用，输入保留；UI重连后手工确认通过 |
| T07 原素材需补语义 | 草稿严格字段校验、预览确认门禁自动检查 | 真实UI清空meaning后预览被拒，补齐后成功；原素材草稿合同通过 |
...
| T10 编辑后旧历史 | 1bf2dac真实Host A1→A2更换文字和图；A1历史与字节hash保持，续聊正确 | Host/模型与最终浏览器新版本编辑、旧A1图片/含义回看均通过 |
...
| T15 隔离/切换/重连 | 真实A/B原生消息隔离；自有核心退出后原Host显式重连成功 | Host与普通A→B→A界面切换通过；未知身份漏洞修补与实际Client反例通过 |
| T16 用户入口/AI展示 | D1/D4原生输入区选择器、主对话图去重 | 最终Client已真实装载，原生入口、历史图和管理操作通过 |
| T17 开箱24项 | 基础包24版本/25素材；真实安装默认库，正式模型使用纯24库 | 库及静态/动画素材通过 |
| T18 私有与导出 | 包白名单检查；实际导出文件仅manifest+所选blob | 通过；最终tarball另核对分发内容 |
| T19 动图/封面/历史 | D1真实静态/动图预览与暂停；D4对话动图；自动失败fallback | 最终包冷恢复动图、暂停封面、真实缺图固定文字fallback及恢复图片通过 |
| T20 故障与未知 | 同Host闲置370042ms；真实退出核心→显式恢复；官方remove/reinstall数据一致 | 核心恢复与原生UI断连恢复通过；跨Session未知/写入丢响应修补有实际Client故障证据 |
...
用户要求重试后，隔离QA页面点击已恢复。已确认草稿直接结束编辑，新版本、缺字段门禁、建议故障、冲突续编、冷恢复与缺图fallback等真实UI补验已通过，见[最终UI补验](dsh-ui-resumed-2026-09-20.md)。没有绕过任何浏览器或原生应用权限。用户日常web profile已实际安装并启动于3080，认证只读catalog27通过。应用内浏览器打开3080报ERR_BLOCKED_BY_CLIENT，未绕过该拒绝；仍未声明日常浏览器点击验收完成。
...
最终支持结论：**dsh 0.1.5-rc.2 / macOS arm64 / Node24的原生表情与管理能力已实现、已安装，并有真实双向模型和主要UI证据；主要原生UI验收已补齐，少数精确故障时序及扩展环境仍只具有合同证据或尚未实测。** 其他版本/OS/PTC、新版Codex/Claude无新增通过声明。上表尚未观察的UI、强制压缩恢复、独立双Host生命周期等部分不得自行变成通过；现有公共合同证据仍有效。

本轮UI补验模型请求增量0；临时缺失blob已原样恢复、QA作品重新归档。旧确认框已不再阻塞隔离QA页，不需要用户重复关闭它。日常3080的应用内浏览器访问仍被拦截，保留该边界。
