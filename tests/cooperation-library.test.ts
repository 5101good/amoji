...
const names=['进度如何','还在吗','卡在哪里','说具体些','举个例子','没看懂','方向不对','先等一下','继续吧','一步一步来','明白了','这个不错','谢谢','一起庆祝'];
test('协作库仅包含14个明确场景，全部素材是真实透明底且文本投影不含图像',async()=>{
 await withValidatedPack(await readFile(seed),async pack=>{
  assert.deepEqual(pack.manifest.expressions.map(e=>e.name),names);
...
test('升级内置库只归档旧内置默认，旧历史/个人副本/用户归档保持且只迁移一次',async t=>{
 const dir=await mkdtemp(join(tmpdir(),'amoji-collaboration-'));let store:LibraryStore|undefined;
 t.after(async()=>{store?.close();await rm(dir,{recursive:true,force:true});});
 store=await LibraryStore.open(dir,oldSeed);const old=store.list()[0]!;const oldDefinition=structuredClone(old);
 ...
 const fresh=store.list().find(e=>e.name==='进度如何')!;const current=store.getEntry(fresh.asset_id);store.setArchived(fresh.asset_id,current.version,true);
