...
if (process.env.AMOJI_RESUME_THREAD) {
  args.pop();
  args.push('resume', process.env.AMOJI_RESUME_THREAD, process.env.AMOJI_PROBE_PICK === '1' ? pickPrompt : '继续 Amoji 纯文本回放测试：只用上一轮表情的文字语义，用一句话说说你理解到的情绪。不读取文件，不打开图片，不调用工具。');
}
...
child.on('exit', async code => {
  ...
  const expected = process.env.AMOJI_RESUME_THREAD && process.env.AMOJI_PROBE_PICK !== '1' ? [] : ['amoji_search', 'amoji_emit', ...(process.env.AMOJI_PROBE_PICK === '1' ? ['amoji_pick'] : [])];
  const successfulTools = expected.every(name => calls.some(c => c.tool === name && c.status === 'completed' && !c.error && !c.result?.isError && !c.result?.is_error));
  const textOnly = wire.length > 0 && wire.every(request => request.parsed && request.inputImageParts === 0 && request.imageDataUrls === 0);
  const passed = code === 0 && successfulTools && textOnly;
  console.log(JSON.stringify({ run, executable, exitCode: code, passed, requests: wire.length, textOnly, calls: calls.map(c => ({ tool: c.tool, status: c.status })), stderr: stderr.slice(-1200) }, null, 2));
